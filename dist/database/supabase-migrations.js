...
exports.createSupabaseTables = void 0;
...
const createSupabaseTables = async () => {
    try {
        ...
        const { error: testError } = await supabase_client_1.supabase
            .from('users')
            .select('count')
            .limit(1);
...
exports.createSupabaseTables = createSupabaseTables;
// Create default admin user
const createDefaultAdmin = async () => {
    try {
        // Check if admin exists
        const { data: existingAdmin } = await supabase_client_1.supabase
            .from('users')
            .select('id')
            .eq('username', 'admin')
            .single();
...
const createDefaultAppSettings = async () => {
    try {
        ...
        const { data: existingSettings } = await supabase_client_1.supabase
            .from('app_settings')
            .select('id')
            .single();
        if (!existingSettings) {
